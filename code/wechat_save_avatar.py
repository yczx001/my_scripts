#!/usr/bin/env python
# coding=utf-8
# Forked from https://github.com/0x5e/wechat-deleted-friends/blob/master/wdf.py
...
def webwxgetcontact():
    ...
    if data:
        for friend in data['MemberList']:
            # 如果有备注或者真名没有符号
            if friend['RemarkName'] or friend['NickName'].find('<') < 0:
                if friend['RemarkName']:
                    file_name = friend['RemarkName']
                else:
                    file_name = friend['NickName']
                file_name = "./touxiang/" + file_name + '.jpg'
                img_url = "https://wx.qq.com%s&type=big" % friend['HeadImgUrl']
                r = myRequests.get(img_url, headers=headers)
